...
*Proposition 2.3.2* (Multiplication is commutative). /Let $n$ and $m$ be natural numbers. Then $n \times m = m \times n$./
...
- $\Vdash$ { Proof by induction on $n$}
  ...
  - $\equiv$ { Definition 2.3.1: $(n\pp) \times m := (n \times m) + m$ }
  ...
  - $\Vdash$ { Proof by induction on $m$}
    - $\bullet$ Base case: show that $(0 \times n) + 0 = 0 \times (n\pp)$
    - $\Vdash$ { Definition 2.3.1: $0 \times m := 0$ }
    - $~~~$ 0 + 0 = 0
    - $\equiv$ { Definition 2.2.1: $0+m := m$ }
    - $~~~$ True
